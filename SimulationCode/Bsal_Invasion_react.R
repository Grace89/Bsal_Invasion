...
# Code Objective:
  # To simulate Bsal invasion dynamics when there is only REACTIVE management
...
param_combos <- read.csv("./ParameterCombinations/parameter_combos_Bsal_react.csv")
...
# After reactive managemnet - Probability of host colonizing a site - same for before and after Bsal invasion
c_H_react    <- param_combos$c_H_react[niter]

# Before any management - Probability of Bsal colonizing a site
c_S    <- param_combos$c_S[niter]

# After reactive managemnet - Probability of Bsal colonizing a site
c_S_react    <- param_combos$c_S_react[niter]

# Before any management - Host survival probability with NO Bsal
phi_Hb <- param_combos$phi_Hb[niter]

# After reactive managemnet - Host survival probability with NO Bsal
phi_Hb_react <- param_combos$phi_Hb_react[niter]

# Before any management - Multiplier for sites with Bsal present (small and large)
...
# After reactive managemnet - Multiplier for sites with Bsal present (small and large)
delta_s_react <- param_combos$delta_s_react[niter]
delta_L_react <- param_combos$delta_L_react[niter]

phi_Hs_react <- phi_Hb_react * delta_s_react
phi_HL_react <- phi_Hb_react * delta_L_react

# Before any management - Bsal extinction probability at a site where it is small
e_S <- param_combos$e_S[niter]

# After reactive managemnet - Bsal extinction probability at a site where it is small
e_S_react <- param_combos$e_S_react[niter]

# Before any management - Bsal extinction probability at a site where it is large
e_L <- param_combos$e_L[niter]

# After reactive managemnet - Bsal extinction probability at a site where it is large
e_L_react <- param_combos$e_L_react[niter]

# Before any management - Probability Bsal grows from small to large
g_S <- param_combos$g_S[niter]

# After reactive managemnet - Probability Bsal grows from small to large
g_S_react <- param_combos$g_S_react[niter]

# Before any management - Probability Bsal declines from large to small
d_L <- param_combos$d_L[niter]

# After reactive managemnet - Probability Bsal declines from large to small
d_L_react <- param_combos$d_L_react[niter]
...
n.occasions <- n.pre.yrs + n.post.Bsal.pre.treat + n.post.Bsal.post.treat
...
# Before any treatement & After Bsal invasion
...
Bsal_trans_mat <- matrix(c(
  ...
  e_L*(1-c_H),    e_L*c_H, (1-e_L)*d_L*c_H, (1-e_L)*(1-d_L)*(1-c_H), (1-e_L)*(1-d_L)*c_H, (1-e_L)*d_L*(1-c_H),
...
# After reactive treatment & Bsal arrival
  # A site moves between these states with probabilities defined in the probability matrix
  # Defining the transition matrix
Bsal_trans_mat_react <- matrix(c(
  (1-c_S_react)*(1-c_H_react),  (1-c_S_react)*c_H_react, c_S_react*(1-c_H_react), c_S_react*c_H_react, 0, 0,
  (1-c_S_react)*(1-phi_Hb_react), (1-c_S_react)*phi_Hb_react, c_S_react*(1-phi_Hb_react), c_S_react*phi_Hb_react, 0, 0,
  e_S_react*(1-c_H_react), e_S_react*c_H_react, (1-e_S_react)*(1-g_S_react)*(1-c_H_react), (1-e_S_react)*(1-g_S_react)*c_H_react, (1-e_S_react)*g_S_react*(1-c_H_react), (1-e_S_react)*g_S_react*c_H_react,
  e_S_react*(1-phi_Hs_react), e_S_react*phi_Hs_react , (1-e_S_react)*(1-g_S_react)*(1-phi_Hs_react), (1-e_S_react)*(1-g_S_react)*phi_Hs_react, (1-e_S_react)*g_S_react*(1-phi_Hs_react), (1-e_S_react)*g_S_react*phi_Hs_react,
  e_L_react*(1-c_H_react),    e_L_react*c_H_react, (1-e_L_react)*d_L_react*c_H_react, (1-e_L_react)*d_L_react*(1-c_H_react), (1-e_L_react)*(1-d_L_react)*c_H_react, (1-e_L_react)*(1-d_L_react)*(1-c_H_react),
  e_L_react*(1-phi_HL_react), e_L_react*phi_HL_react, (1-e_L_react)*d_L_react*(1-phi_HL_react), (1-e_L_react)*d_L_react*phi_HL_react, (1-e_L_react)*(1-d_L_react)*(1-phi_HL_react), (1-e_L_react)*(1-d_L_react)*phi_HL_react
), 
nrow = n.states, ncol = n.states, byrow = TRUE)

# Add column & row names
colnames(Bsal_trans_mat_react) <- rownames(Bsal_trans_mat_react) <- c("bh", "bH", "sh", "sH", "Lh", "LH")

# Make sure that all the rows sum to 1
rowSums(Bsal_trans_mat_react)
...
for(i in 1:n.sites){  
  ...
  for(k in (n.pre.yrs+1):n.post.Bsal.pre.treat){
  ...
  for(k in (n.post.Bsal.pre.treat+1):n.occasions){
    ...
    z[i,k] <- which(rmultinom(1, 1, Bsal_trans_mat_react[z[i,k-1],]) ==1)
